...
public class BookActivationHandler {
    ...
    private static boolean addEnchantmentToBook(ItemStack bookStack, String enchantmentId, ServerPlayerEntity player) {
        try {
            // 在 1.21.10 中，使用 dispatcher 执行命令
            ...
            player.getEntityWorld().getServer().getCommandManager().getDispatcher().execute(
                    reader, player.getCommandSource().withSilent());
    ...
    private static void createActivationParticles(ServerWorld world, PlayerEntity player) {
        ...
        // 使用 DustParticleEffect 替代 ParticleTypes
        // 生成围绕玩家旋转的粒子
        for (int i = 0; i < 360; i += 15) {
            ...
            // 使用 DustParticleEffect 创建粒子效果
            DustParticleEffect enchantEffect = new DustParticleEffect(0xAA00AA, 1.0f);
            world.spawnParticles(enchantEffect,
        ...
        for (int i = 0; i < 50; i++) {
            ...
            // 使用 DustParticleEffect
            DustParticleEffect sparkEffect = new DustParticleEffect(0x00FFFF, 0.8f);
            world.spawnParticles(sparkEffect,
                    playerPos.x, playerPos.y + 0.5, playerPos.z, 1,
                    offsetX, world.random.nextDouble() * 2.0, offsetZ, 0.1);
        }

        // 生成闪光效果 - 使用亮黄色 DustParticleEffect
        DustParticleEffect flashEffect = new DustParticleEffect(0xFFFF00, 1.5f);
        world.spawnParticles(flashEffect,
