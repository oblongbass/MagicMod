...
public class PlayerJoinHandler {
    ...
    public static void register() {
        ServerPlayConnectionEvents.JOIN.register((handler, sender, server) -> {
            ...
            // 先同步魔法级和解锁状态
            try {
                int magicLevel = SimplePlayerDataManager.getMagicLevel(serverPlayer);
                boolean magicUnlocked = SimplePlayerDataManager.isMagicUnlocked(serverPlayer);
                MagicNetworking.sendMagicLevelSync(serverPlayer, magicLevel, magicUnlocked);
                MagicMod.LOGGER.info("[Magic] 已同步玩家 {} 的魔法级: {}/解锁:{}", serverPlayer.getName().getString(), magicLevel, magicUnlocked);

                // 只有在魔法解锁后才同步魔法值和元素
                if (magicUnlocked) {
                    int currentMana = SimplePlayerDataManager.getCurrentMana(serverPlayer);
                    int maxMana = SimplePlayerDataManager.getMaxMana(serverPlayer);
                    int currentElement = SimplePlayerDataManager.getCurrentElement(serverPlayer);
                    MagicNetworking.sendManaSync(serverPlayer, currentMana, maxMana);
                    MagicNetworking.sendElementSync(serverPlayer, currentElement);
                    MagicMod.LOGGER.info("[Magic] 已同步玩家 {} 的魔法值: {}/{}, 元素: {}", serverPlayer.getName().getString(), currentMana, maxMana, currentElement);
                } else {
                    MagicMod.LOGGER.info("[Magic] 玩家 {} 魔法未解锁，跳过魔法值/元素同步", serverPlayer.getName().getString());
                }
            } catch (Exception e) {
                MagicMod.LOGGER.error("[Magic] 同步魔法数据时发生错误", e);
            }
