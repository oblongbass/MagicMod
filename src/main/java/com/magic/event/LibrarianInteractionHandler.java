...
public class LibrarianInteractionHandler {
    ...
    private static boolean isLibrarian(VillagerEntity villager) {
        try {
            MagicMod.LOGGER.info("[Magic] 开始检测村民职业");
            
            VillagerData data = villager.getVillagerData();
            MagicMod.LOGGER.info("[Magic] 获得 VillagerData: {}", data.getClass().getName());
            
            Object profession = data.profession();
            MagicMod.LOGGER.info("[Magic] 获得职业: {}", profession);
            
            String professionName = profession.toString();
            MagicMod.LOGGER.info("[Magic] 职业名称: {}", professionName);
            
            boolean result = professionName.toLowerCase().contains("bookseller") || professionName.toLowerCase().contains("librarian");
            MagicMod.LOGGER.info("[Magic] 检测结果: {}", result);
    ...
    public static void register() {
        UseEntityCallback.EVENT.register((player, world, hand, entity, hitResult) -> {
            ...
            if (entity instanceof VillagerEntity villager) {
                MagicMod.LOGGER.info("[Magic] 检测到村民交互: {}，手: {}, 潜行: {}", 
                    entity.getName().getString(), hand, player.isSneaking());
                ...
                if (player.isSneaking() && hand == Hand.MAIN_HAND) {
                    MagicMod.LOGGER.info("[Magic] 玩家 {} 尝试与村民交互", player.getName().getString());

                    boolean isLibrarian = isLibrarian(villager);
                    MagicMod.LOGGER.info("[Magic] 村民是否为图书管理员: {}", isLibrarian);

                    if (!isLibrarian) {
                        MagicMod.LOGGER.info("[Magic] 非图书管理员，拒绝交互");
                        return ActionResult.PASS;
                    }

                    boolean magicUnlocked = SimplePlayerDataManager.isMagicUnlocked((ServerPlayerEntity) player);
                    MagicMod.LOGGER.info("[Magic] 魔法是否已解锁: {}", magicUnlocked);
                    ...
                    MagicMod.LOGGER.info("[Magic] 开始处理图书管理员交互");
                    handleLibrarianInteraction(player, villager, world);
                } else {
                    MagicMod.LOGGER.info("[Magic] 不是 Shift+右键，忽略交互");
                }
            } else {
                MagicMod.LOGGER.info("[Magic] 不是村民，忽略交互");
            }
    ...
    private static void handleLibrarianInteraction(PlayerEntity player, VillagerEntity villager, World world) {
        ...
        MagicMod.LOGGER.info("[Magic] 图书管理员 {} 已锁定，对话开始", villager.getUuid());
    }
    ...
    public static void clearDialogueState(ServerPlayerEntity player) {
        ...
        if (world instanceof net.minecraft.server.world.ServerWorld serverWorld) {
            ...
            MagicMod.LOGGER.info("[Magic] 对话结束，解锁附近 {} 个村民", villagers.size());
        }
    }
