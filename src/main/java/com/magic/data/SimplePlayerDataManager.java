...
public class SimplePlayerDataManager {
    ...
    public static int getMagicLevel(ServerPlayerEntity player) {
        PlayerData data = getPlayerData(player);
        return data.getMagicLevel();
    }

    public static int getCurrentElement(ServerPlayerEntity player) {
        PlayerData data = getPlayerData(player);
        return data.getCurrentElement();
    }

    public static void setCurrentElement(ServerPlayerEntity player, int element) {
        World world = player.getEntityWorld();
        MagicCircleSavedData savedData = getSavedData(world);
        if (savedData == null) return;
        PlayerData data = savedData.getPlayerData(player.getUuid());
        data.setCurrentElement(element);
        savedData.setMagicCircle(player.getUuid(), data);
        savedData.markDirty();
    }

    public static boolean isMagicUnlocked(ServerPlayerEntity player) {
        PlayerData data = getPlayerData(player);
        return data.isMagicUnlocked();
    }
