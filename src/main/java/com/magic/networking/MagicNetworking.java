...
public class MagicNetworking {
    ...
    public static void initialize() {
        ...
        // 注册 C2S 元素切换
        try {
            PayloadTypeRegistry.playC2S().register(ElementSwitchC2SPayload.ID, ElementSwitchC2SPayload.CODEC);
            MagicMod.LOGGER.info("[Magic] ElementSwitchC2SPayload 注册成功");
        } catch (Exception e) {
            MagicMod.LOGGER.error("[Magic] ElementSwitchC2SPayload 注册失败", e);
        }
        ...
        // 注册 C2S 元素切换处理器
        try {
            net.fabricmc.fabric.api.networking.v1.ServerPlayNetworking.registerGlobalReceiver(ElementSwitchC2SPayload.ID,
                (payload, context) -> {
                    context.server().execute(() -> {
                        var player = context.player();
                        if (player == null) return;
                        com.magic.data.SimplePlayerDataManager.setCurrentElement(player, payload.element());
                        sendElementSync(player, payload.element());
                        MagicMod.LOGGER.info("[Magic] 玩家 {} 通过快捷键切换到元素 {}", player.getName().getString(), payload.element());
                    });
                });
            MagicMod.LOGGER.info("[Magic] 元素切换处理器注册成功");
        } catch (Exception e) {
            MagicMod.LOGGER.error("[Magic] 元素切换处理器注册失败", e);
        }
    }
