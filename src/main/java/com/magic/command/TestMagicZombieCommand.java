...
public class TestMagicZombieCommand {
    ...
    private static int spawnMagicZombie(CommandContext<ServerCommandSource> context) {
        ...
        try {
            ...
            // 创建魔法僵尸实体
            MagicZombieEntity magicZombie = ModEntities.MAGIC_ZOMBIE.create(player.getWorld(), net.minecraft.entity.SpawnReason.COMMAND);
            ...
            if (magicZombie != null) {
                ...
                // 初始化实体
                magicZombie.initialize(player.getWorld(), player.getWorld().getLocalDifficulty(spawnPos), 
                    net.minecraft.entity.SpawnReason.COMMAND, null);
                
                // 添加到世界
                player.getWorld().spawnEntity(magicZombie);
